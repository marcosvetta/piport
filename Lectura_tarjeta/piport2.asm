...
;	contador_ equ 92
   ...
   Transmision_OK equ 100
	...
	Recive equ   30  ;con 0 recive distinto de cero no recive   
   Tiempo_expire equ 31
   Tiempo_expire_total equ 32
   Cuenta_byte equ 33
   Largo_cadena equ 34
   Tiempo_inter_byte equ 35
   Tiempo_ibytel equ 36
   Tiempo_ibyteh equ 37
   Valor_tibytel equ 38
   Valor_tibyteh equ 39
   Tiempo_etl equ 40
   Tiempo_eth equ 41
   Valor_tetl equ 42
   Valor_teth equ 43
   Valor_cuenta_byte equ 44
   T_expire_total equ 45
   Inicio equ 46
...
VERIFICA_INVERTIDO:			
           cjne a,#0BH,FIN_LOOP
           		 ;ALgo raro no envia vuelve a empezar
ENVIAR_CAD:
		call SUMA30
...
;  mov Flag_orden,#0FFH
  ...
  mov @R0,acc
   ...
   mov acc,Largo_cadena
   inc acc
   mov Largo_cadena,acc
...
mov Recive,#0FFh ; Habilita recibe comunicacion serial
mov r0,#START_SEN
;mov r2,Largo_data
;mov Largo_data,#010H
mov Largo_data,#0FH
...
;mov sbuf,ValorLRC 
;
mov Recive,#0FFh ; DEsHabilita ecibe comunicacion serial
...
mov Valor_cuenta_byte,#06H

mov Valor_tibytel,#020H
